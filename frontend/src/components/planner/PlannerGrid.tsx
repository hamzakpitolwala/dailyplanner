...
export const PlannerGrid: FC<PlannerGridProps> = ({
  tasks,
  fixedBlocks,
  plannerDate,
  editTask,
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full h-[600px] overflow-y-auto bg-slate-50/50 rounded-xl border border-border shadow-inner"
    >
        ...
        <div className="relative flex-1 bg-[linear-gradient(to_bottom,#f1f5f9_1px,transparent_1px)]" style={{ backgroundSize: `100% ${60 * PIXELS_PER_MINUTE}px` }}>
          ...
          {activeBlocks.map(block => {
            ...
            return (
              <div 
                key={block.id}
                className="absolute left-0 right-0 bg-slate-200/50 border-l-4 border-slate-400 opacity-60 pointer-events-none flex flex-col justify-center items-center"
                style={{ top, height }}
              >
                <span className="text-slate-500 font-medium tracking-widest uppercase text-sm">{block.name}</span>
              </div>
            );
          })}
