"""Integration tests for the check-in flow (Phase 2)."""
...
def _register_and_login(email: str = "checkin@example.com") -> str:
...
def test_checkin_done():
    ...
    resp = client.post(
        f"/planners/activities/{activity_id}/checkins",
        json={"status": "done", "notes": "Finished on time"},
...
def test_checkin_partial():
    ...
    resp = client.post(
        f"/planners/activities/{activity_id}/checkins",
        json={"status": "partial", "notes": "Got halfway through"},
        headers=_auth_headers(token),
    )
    assert resp.status_code == 201
...
def test_checkin_not_done_with_reason():
...
def test_checkin_not_done_without_required_reason_fails():
...
def test_checkin_not_done_without_reason_ok_when_not_required():
...
def test_checkin_not_done_with_alternate():
...
def test_checkin_not_done_alternate_ignored_when_not_allowed():
...
def test_checkin_updates_activity_status():
    """After a check-in, the activity's status should match the check-in status."""
...
def test_list_checkins():
    """Listing check-ins for an activity should return all of them."""
    ...
    statuses = {c["status"] for c in checkins}
    assert statuses == {"partial", "done"}


def test_get_single_checkin():
    """Getting a single check-in by id should work."""
    ...
    resp = client.get(
        f"/planners/activities/checkins/{checkin_id}",
        headers=_auth_headers(token),
    )
    assert resp.status_code == 200
...
def test_checkin_activity_not_found():
    """Check-in on a non-existent activity should return 404."""
    token = _register_and_login("notfound@example.com")

    resp = client.post(
        "/planners/activities/99999/checkins",
        json={"status": "done"},
        headers=_auth_headers(token),
    )
...
def test_checkin_reason_strips_for_non_not_done():
    """Reason/alternate should be silently stripped when status is not 'not_done'."""
